...
import { BlogSidebarProps } from '../../types/blog/props/component.props';
...
const BlogSidebar: React.FC<BlogSidebarProps> = ({
  ...
  const PostItem: React.FC<{ post: any; showImage?: boolean }> = ({ post, showImage = true }) => (
    <div
      onClick={() => onPostClick?.(post.id)}
      className="flex space-x-3 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700 rounded-lg p-2 -m-2 transition-colors duration-200"
    >
      {showImage && post.images && post.images.length > 0 && (
        <img
          src={post.images[0]}
          alt={post.title}
  ...
  return (
    <div className={`w-full ${className}`}>
      ...
      {popularPosts.length > 0 && (
        <SidebarSection title="Bài viết phổ biến">
      ...
      {recentPosts.length > 0 && (
        <SidebarSection title="Bài viết mới nhất">
          <div className="space-y-4">
            {recentPosts.slice(0, 5).map((post) => (
              <PostItem key={post.id} post={post} showImage={false} />
            ))}
          </div>
        </SidebarSection>
      )}
      ...
      {/* Tags */}
      {tags.length > 0 && (
        <SidebarSection title="Tags phổ biến">
      ...
      {/* Social Links */}
      <SidebarSection title="Theo dõi chúng tôi">
