...
interface TaskLimitExceededModalProps {
  isOpen: boolean;
  onClose: () => void;
  error: TaskLimitExceededError | null;
}

const TaskLimitExceededModal: React.FC<TaskLimitExceededModalProps> = ({
  isOpen,
  onClose,
  error,
}) => {
  ...
  const formatResetTime = (resetTimeISO: string): string => {
    try {
  ...
  return (
    <Modal
      isOpen={true}
      className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-[480px] max-w-[90vw] bg-white rounded-2xl shadow-2xl z-[2000] outline-none"
      ...
      <div className="relative">
        ...
        <div className="p-6">
          ...
          <div className="flex items-start gap-3 mb-4">
            ...
            <div className="flex-1">
              <h3 className="text-lg font-bold text-gray-900">
                {t('task_limit_exceeded_title', 'Task Limit Reached')}
              </h3>
              <p className="text-xs text-gray-600 mt-0.5">
                {t('task_limit_exceeded_subtitle', 'You have reached your daily limit')}
              </p>
            </div>
          </div>
          ...
          <div className="space-y-2 mb-4 p-3 bg-gray-50 rounded-lg border border-gray-200">
            ...
            <div className="flex justify-between items-center">
              <span className="text-xs font-medium text-gray-700">
                {t('daily_limit', 'Daily Task Limit')}
              </span>
              <span className="text-xs font-semibold text-gray-900">
                {error.details.currentLimit} {t('tasks', 'tasks')}
              </span>
            </div>
            <div className="w-full h-px bg-gray-200" />
            <div className="flex justify-between items-center">
              ...
              <span className="text-xs font-semibold text-gray-900">
                {error.details.tasksCreatedToday} / {error.details.currentLimit}
              </span>
            </div>
            <div className="w-full h-px bg-gray-200" />
            <div className="flex justify-between items-center">
              ...
              <span className="text-xs font-semibold text-gray-900">
                {formatResetTime(error.details.resetTime)}
              </span>
            </div>
          </div>
          ...
          <div className="mb-4 p-3 bg-linear-to-br from-blue-50 to-indigo-50 rounded-lg border border-blue-200">
            <h4 className="font-semibold text-blue-900 mb-2 text-sm flex items-center gap-2">
              <span>⚡</span>
              {t('upgrade_suggestion', 'Upgrade Your Plan')}
            </h4>
            <p className="text-xs text-blue-800 mb-2">
              {t('upgrade_to_text', 'Upgrade to')}{' '}
              <span className="font-semibold">{error.upgrade.suggestedLevel}</span>{' '}
              {t('to_get', 'to get')}{' '}
              <span className="font-semibold">
                {error.upgrade.suggestedLimit}{' '}
                {t('tasks_per_day', 'tasks per day')}
              </span>
            </p>
            {error.upgrade.benefits && error.upgrade.benefits.length > 0 && (
              <ul className="space-y-1">
                {error.upgrade.benefits.slice(0, 2).map((benefit, idx) => (
...
export default TaskLimitExceededModal;
