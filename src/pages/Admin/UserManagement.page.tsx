...
// Selection component
const Selection = ({ value, options, onChange, label }: {
    value: string;
    ...
    return (
        <div className="relative w-full z-30" ref={ref}>
            {label && <span className="block text-xs text-[#21b4ca] mb-1 absolute left-3 top-1/20 -translate-y-1/2 bg-white px-1">{label}</span>}
            <button
                type="button"
...
const UserManagement: React.FC = () => {
    ...
    const getScrollContainer = () =>
        window.__adminScrollEl as HTMLElement | null;
            ...
            <div className="min-h-screen bg-gray-50 ">
                <div >
                    ...
                    <div className="bg-white shadow-md    border border-gray-100">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 flex items-center justify-center">
                                ...
                                <div className="relative">
                                    <Selection
                                        ...
                                        onChange={v => handleRoleFilterChange(v as UserRole | 'all')}
                                        label="Role"
                                    />
                                </div>
                                ...
                                <div className="relative">
                                    <Selection
                                        ...
                                        onChange={v => handleStatusFilterChange(v as UserStatus | 'all')}
                                        label="Status"
                                    />
                                </div>
                            </div>
                        ...
                        <div className="p-4 flex justify-between items-center bg-white">
                            <div>
                                {loading ? (
                                    ...
                                    <span className="text-sm text-gray-600">Total Users: <span className="font-medium text-[#21b4ca]">{totalUsers}</span></span>
                                )}
                            </div>
                            <div className="flex items-center">
                        ...
                        <table className="min-w-full divide-y border-t border-gray-200  divide-gray-200" style={{ position: 'relative' }}>
                            <thead className="bg-white  z-10 border-b border-gray-200 ">
                                <tr className='border-b border-gray-200 '>
                                    <th style={{ position: 'sticky', top: -1, zIndex: 10 }} className="px-6 border-b border-gray-200  py-3 text-left text-xs font-medium text-[#21b4ca] uppercase tracking-wider bg-white">
                                        #
                                    </th>
                                    <th style={{ position: 'sticky', top: -1, zIndex: 10 }} className="px-6 border-b border-gray-200 py-3 text-left text-xs font-medium text-[#21b4ca] uppercase tracking-wider bg-white">
                                        User
                                ...
                                {loading ? (
                                    ...
                                    <>
                                        {users.map((user, index) => (
                                            <tr key={user._id} className="hover:bg-[#21b4ca]/5 transition-colors duration-150">
                                                ...
                                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                                    <span className={`px-2 py-1 inline-flex text-xs  w-[80px] text-center leading-5 justify-center items-center  font-semibold rounded-full ${user.role === 'admin' ? 'bg-[#21b4ca]/20 text-[#21b4ca]' : 'bg-gray-100 text-gray-800'}`}>
                                                        {user.role === 'admin' ? 'Admin' : 'User'}
                                                    </span>
                                                </td>
                                                <td className="px-6 py-4 whitespace-nowrap text-sm">
                                                    <span className={`px-2 py-1 inline-flex text-xs w-[80px] text-center leading-5 justify-center items-center font-semibold rounded-full ${user.status === 'active' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                                ...
                                <div>
                                    {loading ? (
                                        ...
                                        <p className="text-sm text-gray-700">
                                            Showing <span className="font-medium text-[#21b4ca]">{(page - 1) * limit + 1}</span> to{" "}
                                            <span className="font-medium text-[#21b4ca]">
                                                {Math.min(page * limit, totalUsers)}
                                            </span>{" "}
                                            of <span className="font-medium text-[#21b4ca]">{totalUsers}</span> results
                                        </p>
                                    )}
                                </div>
                                <div>
                                    {loading ? (
                                        ...
                                        <nav className="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                                            <button
                                                ...
                                                disabled={page === 1}
                                                className={`relative inline-flex items-center px-2 py-2 rounded-l-md border border-gray-300 bg-white text-sm font-medium ${page === 1 ? 'text-gray-300 cursor-not-allowed' : 'text-[#21b4ca] hover:bg-[#21b4ca]/5 transition-colors duration-150'
                                                    }`}
                                            >
                                            ...
                                            {getPaginationArray().map((pageNum, index) => (
                                                pageNum === '...' ? (
                                                    ...
                                                    <button
                                                        ...
                                                        className={`relative inline-flex items-center px-4 py-2 border text-sm font-medium transition-colors duration-150 ${page === pageNum
                                            ...
                                            <button
                                                ...
                                                className={`relative inline-flex items-center px-2 py-2 rounded-r-md border border-gray-300 bg-white text-sm font-medium ${page === totalPages ? 'text-gray-300 cursor-not-allowed' : 'text-[#21b4ca] hover:bg-[#21b4ca]/5 transition-colors duration-150'
                                                    }`}
                                            >
